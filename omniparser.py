...
class Omniparser(object):
    def __init__(self, config: Dict):
        ...
        self.caption_model_processor = get_caption_model_processor(model_name=config['caption_model_name'], model_name_or_path=config['caption_model_path'], device=config['device'])
    ...
    def parse(self, image_path: str):
       ...
       #                 'text': parsed_content_list[i].split(': ')[1], 'type':'text'} for i, (k, coord) in enumerate(label_coordinates.items()) if i < len(parsed_content_list)]
        ...
        #print(return_list)
        return {'labels':return_list,'labeled_img':dino_labled_img}
